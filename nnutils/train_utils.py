...
flags.DEFINE_string('checkpoint_dir', osp.join('/mnt/51eb0667-f71d-4fe0-a83e-beaff24c04fb/vikrant', 'snapshots'),
                    'Root directory for output files')
flags.DEFINE_integer('print_freq', 10, 'scalar logging frequency')
...
class Trainer():
    def __init__(self, opts):
        ...
        self.transform_custom = torchvision.transforms.Compose([
            torchvision.transforms.ToPILImage(),
            torchvision.transforms.RandomHorizontalFlip(p=1.),
            #torchvision.transforms.RandomAffine(degrees=45, translate=(0.15, 0.15), scale=(1.0, 1.2)),
            #torchvision.transforms.ColorJitter(brightness=0.3, contrast=0.3, saturation=0.2, hue=0.2),
    ...
    def save_basis(self, epoch_prefix):
        '''Saves the part basis.'''
        self.save_network(self.part_basis_generator, 'part_basis', epoch_prefix, gpu_id=self.opts.gpu_id)
        return

    def save_amp(self, epoch_prefix):
        '''Saves the amp state.'''
        save_filename = 'amp_state_{}.pt'.format(epoch_prefix)
        save_path = os.path.join(self.save_dir, save_filename)
        torch.save(amp.state_dict(), save_path, gpu_id=self.opts.gpu_id)
        return

    def load_amp(self, epoch_prefix):
        save_filename = 'amp_state_{}.pt'.format(epoch_prefix)
        save_path = os.path.join(self.save_dir, save_filename)
        amp.load_state_dict(torch.load(save_path))
        return
        ...
        if opts.use_sgd:
            ...
            if opts.seg_train:
                self.optimizer_2 = torch.optim.SGD(
                    self.part_basis_generator.parameters(), lr=opts.learning_rate, momentum=opts.beta1)
        else:
            ...
            if opts.seg_train and not opts.query_basis:
                self.optimizer_2 = torch.optim.Adam(
                    self.part_basis_generator.parameters(), lr=opts.learning_rate, betas=(opts.beta1, 0.999))
    ...
    def train(self):
        ...
        total_steps = dataset_size*opts.num_pretrain_epochs
        ...
        for epoch in range(opts.num_pretrain_epochs, opts.num_epochs):
            ...
            for i, batch in enumerate(self.dataloader):
                ...
                if not self.invalid_batch:
                    ...
                    with torch.cuda.amp.autocast():
                        self.forward(total_steps)
                        self.smoothed_total_loss = self.smoothed_total_loss * 0.99 + 0.01 * self.total_loss.item()
                    # with amp.scale_loss(self.total_loss, [self.optimizer, self.optimizer_2]) as scaled_loss:
                    ...
                    #bwd_times.append(time.time()-bwd_start_time)

                    scaler.unscale_(self.optimizer)
                    torch.nn.utils.clip_grad_norm_(self.model.parameters(), 0.1) #5.0 earlier
                    scaler.step(self.optimizer)  # .step()

                    if opts.seg_train and not opts.query_basis:
                        scaler.unscale_(self.optimizer_2)
                        print(self.part_basis_generator.w.grad)
                        torch.nn.utils.clip_grad_norm_(self.part_basis_generator.parameters(), 0.1) #5.0 earlier
                        scaler.step(self.optimizer_2)  # .step()
                ...
                if opts.display_visuals and (total_steps % opts.display_freq == 0):
                    ...
                    print(total_steps, ': visualization iter')
                ...
                if total_steps % opts.save_latest_freq == 0:
                    ...
                    self.save('latest')
                    if opts.seg_train and not opts.query_basis:
                        print('saving the part basis at the end of epoch {:d}, iters {:d}'.format(epoch, total_steps))
                        self.save_basis('latest')
                    #self.save_amp(epoch+1)

                if total_steps == opts.num_iter:
                    return

                if i / dataset_size > 0.1:
                    break
            ...
            if (epoch + 1) % opts.save_epoch_freq == 0:
                ...
                if opts.seg_train and not opts.query_basis:
                    print('saving the part basis at the end of epoch {:d}, iters {:d}'.format(epoch, total_steps))
                    self.save_basis('latest')
                    self.save_basis(epoch + 1)

                #self.save_amp(epoch+1)
            # return [fwd_times, bwd_times, proj_times, mask_times, tex_times,seg_times, loss_times]
