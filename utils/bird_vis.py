...
class VisRenderer(object):
    ...
    def __call__(self, verts, cams=None, texture=None, rend_mask=False):
        ...
        if texture.dim() == 3:
            # Here input is F x T * T x 3)
            # So add batch dim.
            nf, tex_area = texture.shape[:2]
            tex_size = np.sqrt(tex_area).astype('int32')
            texture = texture.permute(0, 2, 1).view(nf, 3, tex_size, tex_size) # 1 x F x 3 x T x T
            texture = texture.unsqueeze(-1).repeat(1, 1, 1, 1, tex_size)
            texture = texture.permute(0, 2, 3, 4, 1)

            #nf, ts = texture.shape[:2]
            #texture = texture.view(bs, nf, ts**3, 3)        
        if texture.dim() == 5:
    ...
    def saveMesh(self, vert, texture=None):
        if texture is None:
            texture = self.default_tex
        if texture.dim() == 3:
            # Here input is F x T * T x 3)
        ...
        neural_renderer.save_obj('demo.obj', vert, self.faces[0], texture[0].contiguous())
